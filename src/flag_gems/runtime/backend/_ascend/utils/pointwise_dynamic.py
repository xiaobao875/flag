...
class KernelGenerator:
        ...
        if self.ndim == 2:
            ...
            with code.indent():
                ...
                with code.indent():
                    code.writeline(
                        """
                        triton.Config({'tile_size1':128}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':256}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':512}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':1024}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':2048}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':4096}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':8192}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':16384}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':21760}, num_stages=3, num_warps=1),
                        triton.Config({'tile_size1':32768}, num_stages=3, num_warps=1)
                        """
                    )
                code.writeline("],")
                code.writeline("key=['num_tasks'],")
                code.writeline("prune_configs_by={'early_config_prune': early_config_prune},")
            code.writeline(")")
    ...
    def gen_signature(self, code, with_block_pointer=False, ascend=False):
    ...
    def gen_body_gsl_without_bptr(self, code):
        ...
        if self.ndim == 1:
            ...
            code.writeline("tiles_per_cta = tl.cdiv(BLOCK_SIZE, tile_size0)")
        code.writeline("for j in range(0, tiles_per_cta):")
        with code.indent():
            code.writeline("tile_id = pid + j * num_ctas")
    ...
    def codegen_nd_tile_with_bptr(self, code):
        ...
        self.gen_signature(code, with_block_pointer=True, ascend=True)
    ...
    def gen_kernel_2d_ascend(self, code: IndentedBuffer):
        ndim = self.ndim
        schema = self.fx
        code.writeline('def early_config_prune_2d(configs, named_args, **kwargs):')
        with code.indent():
            code.writeline('numel = named_args["num_tasks"]')
            code.writeline('shape0 = named_args["s0"]')
            code.writeline('shape1 = named_args["s1"]')
            code.writeline('in0 = named_args.get("in0_ptr")')
            code.writeline('dtype = in0.dtype')

            # dtype_to_bits mapping
            code.writeline('dtype_to_bits = {')
            with code.indent():
                code.writeline('torch.float32: 32,')
                code.writeline('torch.int32: 32,')
                code.writeline('torch.int64: 64,')
                code.writeline('torch.float16: 16,')
                code.writeline('torch.bfloat16: 16,')
                code.writeline('torch.int16: 16,')
                code.writeline('torch.int8: 8,')
                code.writeline('torch.uint8: 8,')
                code.writeline('torch.bool: 8,')
            code.writeline('}')
            code.writeline('element_bits = dtype_to_bits.get(dtype, dtype_to_bits[dtype])')
            code.writeline('UB_CAPACITY_BITS = 1572864')  # Ascend NPU UB size

            code.writeline('pruned_configs = []')
            code.writeline('for config in configs:')
            with code.indent():
                code.writeline('tile_size0 = config.kwargs["tile_size0"]')
                code.writeline('tile_size1 = config.kwargs["tile_size1"]')
                code.writeline('num_tiles0 = triton.cdiv(shape0, tile_size0)')
                code.writeline('num_tiles1 = triton.cdiv(shape1, tile_size1)')
                code.writeline('num_ctas = min(48 // 1, num_tiles0 * num_tiles1)')
                code.writeline('required_bits = tile_size0 * tile_size1 * element_bits')
                code.writeline('grid = num_ctas')
                code.writeline('if grid > 65535:')
                with code.indent():
                    code.writeline('print(f"Prune config with tile_size1: {config.kwargs[\'tile_size1\']}")')
                    code.writeline('continue')
                code.writeline('if tile_size0 * tile_size1 > 1048576:')
                with code.indent():
                    code.writeline('print(f"Prune config: tile_size0={{tile_size0}}, tile_size1={{tile_size1}} "')
                    code.writeline('      f"-> tile_size0 * tile_size1 = {{tile_size0 * tile_size1}} > 1048576")')
                    code.writeline('continue')
                # code.writeline('if required_bits > UB_CAPACITY_BITS:')
                # with code.indent():
                #     code.writeline('print(f"Prune config due to UB overflow: "')
                #     code.writeline('      f"tile_size0={{tile_size0}}, tile_size1={{tile_size1}}, "')
                #     code.writeline('      f"dtype={{dtype}}, "')
                #     code.writeline('      f"required_bits={{required_bits}} > {{UB_CAPACITY_BITS}}")')
                #     code.writeline('continue')
                code.writeline('pruned_configs.append(config)')
            code.writeline('if len(pruned_configs) == 0:')
            with code.indent():
                code.writeline('print("[WARNING] No valid configs")')
            code.writeline('return pruned_configs')
        ...
        with code.indent():
            code.writeline('configs=[')
            with code.indent():
                code.writeline('triton.Config({\'tile_size0\':32, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':64, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':128, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':256, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':512, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':1024, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':2048, \'tile_size1\':1}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\':4096, \'tile_size1\':1}, num_stages=3, num_warps=1),')
            code.writeline('],')
            code.writeline('key=[\'num_tasks\'],')
            code.writeline('prune_configs_by={\'early_config_prune\': early_config_prune_2d},')
        code.writeline(')')
        code.writeline('@triton.jit')
        # code.writeline('def copy_func_kernel_rank_2(')
        # with code.indent():
        #     code.writeline('in0_ptr: tl.tensor,  # of tl.pointer_type')
        #     code.writeline('out0_ptr: tl.tensor,  # of tl.pointer_type')
        #     code.writeline('in0_stride0: int, in0_stride1: int,  # strides for in0')
        #     code.writeline('out0_stride0: int, out0_stride1: int,  # strides for out0')
        #     code.writeline('s0: int, s1: int,  # task_space')
        #     code.writeline('num_tasks: int,')
        #     code.writeline('tile_size0: tl.constexpr,')
        #     code.writeline('tile_size1: tl.constexpr,')
        #     code.writeline('# tiles_per_cta: int,')
        #     code.writeline('# one_tile_per_cta: tl.constexpr,')
        # code.writeline('):')
        self.gen_signature(code, with_block_pointer=False, ascend=True)

        # 函数体
        with code.indent():
            code.writeline('tl.static_print("tile_size0:", tile_size0)')
            code.writeline('tl.static_print("tile_size1:", tile_size1)')
            code.writeline('pid = tle.program_id(0)')
            code.writeline('num_tiles0 = tl.cdiv(s0, tile_size0)')
            code.writeline('num_tiles1 = tl.cdiv(s1, tile_size1)')
            code.writeline('num_ctas = tl.num_programs(0)')
            code.writeline('')
            code.writeline('tiles_per_cta = tl.cdiv(num_tiles0 * num_tiles1, min(48//1, num_tiles0 * num_tiles1))')
            code.writeline('for j in range(0, tiles_per_cta)😂
            with code.indent():
                code.writeline('tile_id = pid + j * num_ctas')
                code.writeline('# pid multi index recontruction: we use c ordering, right axes changes fastest')
                code.writeline('tile_id1 = tile_id % num_tiles1')
                code.writeline('tile_id //= num_tiles1')
                code.writeline('tile_id0 = tile_id')
                code.writeline('')
                code.writeline('offsets0 = tile_id0 * tile_size0 + tl.arange(0, tile_size0)')
                code.writeline('offsets1 = tile_id1 * tile_size1 + tl.arange(0, tile_size1)')
                code.writeline('mask0 = offsets0 < s0')
                code.writeline('mask1 = offsets1 < s1')
                code.writeline('mask = mask0[:, None] & mask1[None, :]')
                # code.writeline('# loads')
                # code.writeline('in0 = tl.load(in0_ptr + offsets0[:, None] * in0_stride0 + offsets1[None, :] * in0_stride1, mask=mask).to(in0_ptr.type.element_ty)')
                # code.writeline('')
                # loads
                code.writeline("# loads")
                for i in range(schema.num_input_tensors()):
                ...
                code.writeline('')
                # code.writeline('# store')
                # code.writeline('tl.store(out0_ptr + offsets0[:, None] * out0_stride0 + offsets1[None, :] * out0_stride1, out0, mask=mask)')
                # stores
                for i in range(schema.num_output_tensors()):
                    offsets = tuple(
                        f"offsets{j}{_broadcast_vec(j, ndim)} * out{i}_stride{j}"
    ...
    def gen_kernel_3d_ascend(self, code: IndentedBuffer):
        ndim = self.ndim
        schema = self.fx
        code.writeline('def early_config_prune_3d(configs, named_args, **kwargs):')
        with code.indent():
            code.writeline('numel = named_args["num_tasks"]')
            code.writeline('shape0 = named_args["s0"]')
            code.writeline('shape1 = named_args["s1"]')
            code.writeline('shape2 = named_args["s2"]')
            ...
            with code.indent():
                ...
                code.writeline('torch.bool: 8,')
            code.writeline('}')
            code.writeline('element_bits = dtype_to_bits.get(dtype, 32)')
            code.writeline('UB_CAPACITY_BITS = 1572864  # 1.5MB in bits')
            ...
            with code.indent():
                code.writeline('tile_size0 = config.kwargs["tile_size0"]')
                code.writeline('tile_size1 = config.kwargs["tile_size1"]')
                code.writeline('tile_size2 = config.kwargs["tile_size2"]')

                code.writeline('num_tiles0 = triton.cdiv(shape0, tile_size0)')
                code.writeline('num_tiles1 = triton.cdiv(shape1, tile_size1)')
                code.writeline('num_tiles2 = triton.cdiv(shape2, tile_size2)')
                code.writeline('num_ctas = min(48 // 1, num_tiles0 * num_tiles1 * num_tiles2)')

                code.writeline('total_tile_elements = tile_size0 * tile_size1 * tile_size2')
                code.writeline('required_bits = total_tile_elements * element_bits')

                # Grid size limit
                code.writeline('# 1. Grid size limit')
                code.writeline('if num_ctas > 65535:')
                with code.indent():
                    code.writeline('print(f"Prune config due to grid overflow: num_ctas={{num_ctas}} > 65535")')
                    code.writeline('continue')

                # Tile too large
                code.writeline('# 2. Tile too large')
                code.writeline('if total_tile_elements > 1048576:  # 1M elements limit')
                with code.indent():
                    code.writeline('print(f"Prune config: tile too large -> {{tile_size0}}x{{tile_size1}}x{{tile_size2}} = {{total_tile_elements}} > 1048576")')
                    code.writeline('continue')

                # UB memory overflow
                code.writeline('# 3. UB overflow')
                code.writeline('if required_bits > UB_CAPACITY_BITS:')
                with code.indent():
                    code.writeline('print(f"Prune config due to UB overflow: "')
                    code.writeline('      f"tile={{tile_size0}}x{{tile_size1}}x{{tile_size2}}, dtype={{dtype}}, "')
                    code.writeline('      f"required_bits={{required_bits}} > {{UB_CAPACITY_BITS}}")')
                    code.writeline('continue')
            ...
            code.writeline('if len(pruned_configs) == 0:')
            with code.indent():
                code.writeline('print("[WARNING] No valid configs after pruning for 3D kernel!")')
        ...
        with code.indent():
            code.writeline('configs=[')
            with code.indent():
                code.writeline('triton.Config({\'tile_size0\': 8,  \'tile_size1\': 8,  \'tile_size2\': 8},  num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 16,  \'tile_size1\': 16,  \'tile_size2\': 16},  num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 32,  \'tile_size1\': 32,  \'tile_size2\': 32},  num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 32,  \'tile_size1\': 32,  \'tile_size2\': 64},  num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 32,  \'tile_size1\': 64,  \'tile_size2\': 64},  num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 64,  \'tile_size1\': 64,  \'tile_size2\': 64},  num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 64,  \'tile_size1\': 64,  \'tile_size2\': 128}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 64,  \'tile_size1\': 128, \'tile_size2\': 128}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 128, \'tile_size1\': 128, \'tile_size2\': 128}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 128, \'tile_size1\': 128, \'tile_size2\': 256}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 256, \'tile_size1\': 256, \'tile_size2\': 256}, num_stages=3, num_warps=1),')
                code.writeline('triton.Config({\'tile_size0\': 512, \'tile_size1\': 512, \'tile_size2\': 512}, num_stages=3, num_warps=1),')
            code.writeline('],')
            code.writeline('key=[\'num_tasks\'],')
            code.writeline('prune_configs_by={\'early_config_prune\': early_config_prune_3d},')
        code.writeline(')')
        code.writeline('@triton.jit')
        # code.writeline('def copy_func_kernel_rank_3(')
        # with code.indent():
        #     code.writeline('in0_ptr: tl.tensor,')
        #     code.writeline('out0_ptr: tl.tensor,')
        #     code.writeline('in0_stride0: int, in0_stride1: int, in0_stride2: int,')
        #     code.writeline('out0_stride0: int, out0_stride1: int, out0_stride2: int,')
        #     code.writeline('s0: int, s1: int, s2: int,')
        #     code.writeline('num_tasks: int,')
        #     code.writeline('tile_size0: tl.constexpr, tile_size1: tl.constexpr, tile_size2: tl.constexpr,')
        # code.writeline('):')
        self.gen_signature(code, with_block_pointer=False, ascend=True)
        ...
        with code.indent():
            code.writeline('tl.static_print("tile_size0:", tile_size0)')
            code.writeline('tl.static_print("tile_size1:", tile_size1)')
            code.writeline('tl.static_print("tile_size2:", tile_size2)')
            code.writeline('')
            code.writeline('pid = tle.program_id(0)')
            code.writeline('num_tiles0 = tl.cdiv(s0, tile_size0)')
            code.writeline('num_tiles1 = tl.cdiv(s1, tile_size1)')
            code.writeline('num_tiles2 = tl.cdiv(s2, tile_size2)')
            code.writeline('num_ctas = tl.num_programs(0)')
            code.writeline('')
            code.writeline('# Total tiles and actual CTAs')
            code.writeline('total_tiles = num_tiles0 * num_tiles1 * num_tiles2')
            code.writeline('num_ctas_actual = min(48 // 1, total_tiles)  # Match grid_cal logic')
            code.writeline('tiles_per_cta = tl.cdiv(total_tiles, num_ctas_actual)')
            code.writeline('')
            code.writeline('for j in range(tiles_per_cta):')
            with code.indent():
                code.writeline('tile_id = pid + j * num_ctas')
                code.writeline('')
                code.writeline('# C-order indexing: rightmost axis changes fastest')
                code.writeline('tmp = tile_id')
                code.writeline('tile_id2 = tmp % num_tiles2')
                code.writeline('tmp = tmp // num_tiles2')
                code.writeline('tile_id1 = tmp % num_tiles1')
                code.writeline('tmp = tmp // num_tiles1')
                code.writeline('tile_id0 = tmp')
                code.writeline('')
                code.writeline('offsets0 = tile_id0 * tile_size0 + tl.arange(0, tile_size0)')
                code.writeline('offsets1 = tile_id1 * tile_size1 + tl.arange(0, tile_size1)')
                code.writeline('offsets2 = tile_id2 * tile_size2 + tl.arange(0, tile_size2)')
                code.writeline('')
                code.writeline('mask0 = offsets0 < s0')
                code.writeline('mask1 = offsets1 < s1')
                code.writeline('mask2 = offsets2 < s2')
                code.writeline('')
                code.writeline('mask = mask0[:, None, None] & mask1[None, :, None] & mask2[None, None, :]')
                code.writeline('')
                # code.writeline('# Load from input')
                # code.writeline('in0 = tl.load(')
                # with code.indent():
                #     code.writeline('in0_ptr +')
                #     code.writeline('offsets0[:, None, None] * in0_stride0 +')
                #     code.writeline('offsets1[None, :, None] * in0_stride1 +')
                #     code.writeline('offsets2[None, None, :] * in0_stride2,')
                #     code.writeline('mask=mask')
                # code.writeline(').to(in0_ptr.type.element_ty)')
                # code.writeline('')
    ...
    def codegen_nd_tile(self, code):
        if self.ndim == 2:
            self.gen_import_function(code)
            self.gen_kernel_2d_ascend(code)
            return code
        elif self.ndim == 3:
            self.gen_import_function(code)
            self.gen_kernel_3d_ascend(code)
            return code
...
class WrapperGenerator:
    ...
    def gen_task_partition(self, code: IndentedBuffer):
        ...
        if ndim == 0:
            ...
            fn_name = self.jit_fn_name
            if "copy_func_kernel_rank_4" in fn_name:
                code.writeline(
                    f"tile_sizes = heuristics_for_tile_size(2048, *shape)"
                )
            else:
                code.writeline(
        ...
        if ndim == 1:
            max_grid_size0 = self.config.max_grid_size[0]
            code.writeline(
                # f"grid = lambda meta: (min(65535, triton.cdiv(num_tasks, meta['tile_size0'])),)"
                f"grid = lambda meta: (triton.cdiv(num_tasks, meta['BLOCK_SIZE']), 1, 1)"
            )
        else:
    ...
    def gen_wrapper_2d_ascend(self, code: IndentedBuffer):
        # 函数定义
        # code.writeline('def copy_func_wrapper_rank_2(in0: Union[torch.Tensor, StridedBuffer], /, *, out0: Union[torch.Tensor, StridedBuffer]):')
        with code.indent():
            # docstring
            code.writeline('"""Generated wrapper function with Pointwise: StridedBuffer, StridedBuffer(a1!) -> StridedBuffer(a1!)"""')

            # shape assert
            code.writeline('assert in0.shape == out0.shape, \'operand shapes mismatch\'')

            code.writeline('print("enter yangbochu 2!")')

            # task partitioning comment
            code.writeline('# task partitioning')
            code.writeline('shape = out0.shape')
            code.writeline('num_tasks = out0.numel()')
            code.writeline('if num_tasks == 0:')
            ...
            # grid_cal function definition
            code.writeline('def grid_cal(meta):')
            with code.indent():
                code.writeline('block_size0 = meta[\'tile_size0\']')
                code.writeline('block_size1 = meta[\'tile_size1\']')
                code.writeline('num_tiles0 = triton.cdiv(shape[0], block_size0)')
                code.writeline('num_tiles1 = triton.cdiv(shape[1], block_size1)')
                code.writeline('num_ctas = min(48 // 1, num_tiles0 * num_tiles1)')
                code.writeline('return (num_ctas, 1, 1)')

            # set grid
            code.writeline('grid = grid_cal')

            # kernel launch comment
            # code.writeline('# kernel launch')
            ...
            #         ')'
            #     ]
            #     for line in kernel_launch_lines:
            #         code.writeline(line)
            self.gen_kernel_launch(code, ascend=True)

            # return statement
            # code.writeline('return out0, out1')

    def gen_wrapper_3d_ascend(self, code: IndentedBuffer):
    # 函数定义
        # code.writeline('def copy_func_wrapper_rank_3(in0: Union[torch.Tensor, StridedBuffer], /, *, out0: Union[torch.Tensor, StridedBuffer]):')
        with code.indent():
            # docstring
            code.writeline('"""Generated wrapper function with Pointwise: StridedBuffer, StridedBuffer(a1!) -> StridedBuffer(a1!)"""')
            ...
            # shape assert
            code.writeline('assert in0.shape == out0.shape, \'operand shapes mismatch\'')
            code.writeline('print("enter yangbochu 3!")')
            ...
            with code.indent():
                code.writeline('block_size0 = meta[\'tile_size0\']')
                code.writeline('block_size1 = meta[\'tile_size1\']')
                code.writeline('block_size2 = meta[\'tile_size2\']')
                code.writeline('num_tiles0 = triton.cdiv(shape[0], block_size0)')
                code.writeline('num_tiles1 = triton.cdiv(shape[1], block_size1)')
                code.writeline('num_tiles2 = triton.cdiv(shape[2], block_size2)')
                code.writeline('num_ctas = min(48 // 1, num_tiles0 * num_tiles1 * num_tiles2)  # num_warps=1 fixed')
                code.writeline('return (num_ctas, 1, 1)')
    ...
    def codegen_nd_tile(self, code):
        if self.ndim == 2:
            ...
            self.gen_wrapper_2d_ascend(code)
            with code.indent():
                self.gen_return(code)
            return code
        elif self.ndim == 3:
            self.gen_signature(code)
            self.gen_wrapper_3d_ascend(code)
            with code.indent():
                self.gen_return(code)
            return code
        self.gen_signature(code)
...
class PointwiseDynamicFunction:
    ...
    def __call__(self, *args, **kwargs):
        logging.debug("ASCEND POINTWISE TEMPLATE")
    ...
    def instantiate(self, ndim):
        ...
        spec = importlib.util.spec_from_file_location(
            f"gen_module{self._scalar_fn_cache_key}rank{ndim}pid{self.pid}",
            f.name,
        )
...
def pointwise_dynamic(
    ...
    *,
    ...
    def decorator(fn):
        print("enter Ascend pointwise_dynamic!")
        nonlocal num_inputs
